...
import { createMedicineAPI } from "../api/Medicines";
...
export default function MedicineForm() {
  ...
  const onSubmitCreateMedicine = handleSubmit(async (data) => {
    console.log(data, "dataaa");
    const res = await createMedicineAPI(data);
    console.log(res, "res");
    if (!res) {
  ...
  return (
    <form
      onSubmit={onSubmitCreateMedicine}
    ...
    >
      ...
      <div className="flex flex-row mb-2">
        ...
        <button type="submit" disabled={errors.medicine?.type} className="mr-1">
          <AddBtn />
        </button>
      </div>
